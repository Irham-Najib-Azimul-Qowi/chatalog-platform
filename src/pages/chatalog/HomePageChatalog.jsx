...
import { useAuth } from '../../hooks/useAuth'; // Impor useAuth untuk Hero Section
...
// Homepage Web Utama Chatalog (Versi Desain Ulang v3 - Lengkap)
function HomePageChatalog({ isPreview = false, previewData = {} }) {
  ...
  const isLoggedIn = !isPreview && currentUser;
  ...
  const data = isPreview ? previewData : defaultData;

  // --- KODE LENGKAP UNTUK FEATURE CARD ---
  ...
  // --- KODE LENGKAP UNTUK TESTIMONIAL CARD ---
  ...
  return (
    <>
      {/* ================================================== */}
      {/* Bagian 1: Hero Section (LOGIKA BARU)             */}
      {/* ================================================== */}
      {isLoggedIn ? (
        // --- TAMPILAN JIKA SUDAH LOGIN (Hero Full Width) ---
        <section 
          className="h-[40vh] md:h-[50vh] bg-cover bg-center flex items-center justify-center text-white" 
          style={{ backgroundImage: `linear-gradient(rgba(0,0,0,0.5), rgba(0,0,0,0.5)), url('https://images.unsplash.com/photo-1519389950473-47ba0277781c?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2070&q=80')` }}
        >
          <div className="container mx-auto px-6 text-center">
            <h1 className="text-4xl md:text-5xl font-extrabold mb-4 leading-tight">
              Selamat Datang Kembali, {userData?.name || 'Admin'}!
            </h1>
            <p className="text-lg md:text-xl opacity-90 max-w-2xl mx-auto">
              {userData?.role === 'superadmin' 
                ? "Kelola platform Chatalog dari menu Superadmin di atas." 
                : "Anda bisa mengelola toko Anda dengan masuk ke Editor."
              }
            </p>
            {userData?.role === 'toko_admin' && (
              <Link 
                to="/editor" 
                className="mt-8 inline-block bg-chatalog-secondary text-black font-bold py-3 px-8 rounded-lg text-lg 
                           hover:opacity-80 transition-all duration-300
                           transform hover:scale-105 shadow-lg"
              >
                Masuk ke Editor Toko
              </Link>
            )}
          </div>
        </section>
      ) : (
        // --- TAMPILAN JIKA BELUM LOGIN (Split Layout) ---
        <section className="bg-white">
          <div className="container mx-auto px-6 py-20 md:py-32 grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
            {/* Kiri: Teks Konten (Live) */}
            <div className="text-left">
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <Link to="/register" className="bg-chatalog-secondary text-black font-bold py-3 px-8 rounded-lg text-lg 
                           hover:opacity-80 transition-all duration-300
                           transform hover:scale-105 shadow-lg hover:shadow-xl text-center">
                  Daftar Sekarang
                </Link>
                <Link to="/simulator" className="bg-transparent border-2 border-chatalog-primary text-chatalog-primary font-bold py-3 px-8 rounded-lg text-lg 
            ...
            <div className="flex justify-center">
              <div className="w-full h-full max-h-[500px] min-h-[400px] bg-placeholder rounded-lg shadow-2xl"></div>
            </div>
          </div>
        </section>
      )}
      ...
      <section ref={countUpRef} className="bg-background-light py-20">
        <div className="container mx-auto px-6 text-center">
          ...
          <div className="text-7xl md:text-8xl font-extrabold text-chatalog-primary my-4">
            {jumlahToko}+
          </div>
